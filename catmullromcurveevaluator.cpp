...
void CatmullRomCurveEvaluator::evaluateCurve(const std::vector<Point>& ptvCtrlPts, std::vector<Point>& ptvEvaluatedCurvePts,
										 const float& fAniLength, const bool& bWrap) const {
	ptvEvaluatedCurvePts.clear();
	vector<Point> temp_pt_arr;
	int ctrlPointsSize = ptvCtrlPts.size();
	const Mat4d M(-1, 3, -3, 1, 2, -5, 4, -1, -1, 0, 1, 0, 0, 2, 0, 0);
	if (!bWrap) {
	...
	for (; i + 3 < temp_pt_arr.size(); i++) {
		...
		for (int j = 0; j < seg_num; ++j) {
			...
			Vec4d TM = (M.transpose() * T)/2;
			Point temp_pt(TM * px, TM * py);
			if (!ptvEvaluatedCurvePts.empty()) {
				if (temp_pt.x > ptvEvaluatedCurvePts.back().x)
